...
def _handle_dep(pkg: DependencyNode):
    ...
    # (if pkg depends on pkgB which is installed by its child pkgC, when removing pkgC pkgB becomes child of pkg)
    while pkg.children:
...
def remove(pkg_id: str, by_user: bool = True):
    ...
    Args:
        ...
        by_user (bool, optional): Was remove requested by user directly? If True, \
            user will be asked to confirm removal for non-top-level packages
    """
    try:
